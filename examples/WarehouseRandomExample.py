...
def main():
    ...
    for agent_id in env.action_space.spaces.keys():
        randomAgents.append(RandomAgent(agentId=agent_id, environment=env))
    complexAgent = BasicComplexAgent(randomAgents)
    experiment = Experiment(complexAgent, env, maxSteps=100, seedlist=np.arange(0,4000,10), render=True, renderDelay=0.5)
