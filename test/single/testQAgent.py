...
class testQAgent(LoggedTestCase):
    ...
    env = Mock(spec=Env)
    env.action_space = Dict({ENTITY1:Discrete(3)})
    
    def test_smoke(self):
        QAgent(ENTITY1, self.env)
        
    def test_getQ(self):
        agent = QAgent(ENTITY1, self.env)
        self.assertEqual(0, agent._getQ(STATE1, ACT1))

    def test_updateQ_step1(self):
        agent = QAgent(ENTITY1, self.env, {'alpha':0.1, 'gamma':1, 'm':500, 's':0.01})
        agent._updateQ(STATE2, ACT2, STATE3, 2)
        self.assertEqual(0.2, agent._getQ(STATE2, ACT2))
    
    def test_updateQ_step2(self):
        agent = QAgent(ENTITY1, self.env, {'alpha':0.1, 'gamma':.9, 'm':500, 's':0.01})
        agent._updateQ(STATE2, ACT2, STATE3, 2)
    ...
    def test_chooseActionBest(self):
        ...
        agent = QAgent(ENTITY1, self.env, {'alpha':0.1, 'gamma':1, 'm':-500, 's':1})
        agent._updateQ(STATE1, ACT1, STATE2, 2)
        agent._updateQ(STATE1, ACT2, STATE3, 3)
        for i in range(10):
