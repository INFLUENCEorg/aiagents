...
class ReplayMemory(dict):
    ...
    def __getitem__(self, key):
        if key not in self.keys():
            self[key] = list()
        return super(ReplayMemory, self).__getitem__(key)
    ...
    def store(self, path):
        ...
        """
        with open(path + 'replay_memory.pkl', 'wb') as f:
...
class SerialSampling(ReplayMemory):
    """
...
class RandomSequenceSampling(ReplayMemory):
    """
